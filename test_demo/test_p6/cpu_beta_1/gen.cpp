...
struct mips_lTypeSet//id=3
{
	...
	{
		int addr=(hardWare.tmpgrf[base]+offset)&4095;
		if (num==0)
		{
	...
	inline bool trydo(int a[])
	{
		a[3]=((rand()%40)<<2)+(hardWare.tmpgrf[a[2]]&4095);
		if ((a[3]&0xfffff800)!=0 && ((~a[3])&0xfffff800)!=0)
		{
...
struct mips_sTypeSet//id=4
{
	...
	inline void debug()
	{
		...
		{
			hardWare.tmpdm[((a+b)&4095)>>2]=0x11111111;
			if (n==-1) return;
	...
	inline bool trydo(int a[])
	{
		a[3]=((rand()%50+70)<<2)+(hardWare.tmpgrf[a[2]]&4095);
		if ((a[3]&0xfffff800)!=0 && ((~a[3])&0xfffff800)!=0)
		{
			if (DEBUGMOOD) fprintf(out,"failed %s $%d,%d($%d)\n",
			instruction[a[0]],a[1],a[3],a[2]);
