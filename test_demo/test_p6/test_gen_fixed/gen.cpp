...
const int getInstrNum=10,useInstrNum=12,topNum=0x3e00;
FILE *out,*fpr; int fprtot=1,tagtot,buffertot,lanum,laper;
...
char dmBufferNormal[4005],dmBufferJump[4005];
...
struct mips_lTypeSet//id=3
{
	...
	{
		int addr=(hardWare.tmpgrf[base]+offset);
		if (num==0)
		{
	...
	{
		/*
		if(hardWare.tmpgrf[a[2]]>=67107840 || hardWare.tmpgrf[a[2]]<=-67107840){
			a[2] = 0;
			a[3] = ((rand()%40)<<2);
			doit(a[0],a[1],a[2],a[3]);
			hardWare.tmppc+=4;
		...
		a[3]=(abs(rand()%40)<<2)-(hardWare.tmpgrf[a[2]]);
		if ((0<=(hardWare.tmpgrf[a[2]]+a[3]) && (hardWare.tmpgrf[a[2]]+a[3])<=0x2fff) &&
			((a[3]&0xffff8000)==0 || ((~a[3])&0xffff8000)==0) &&
			(a[0]==0 || a[0]==1 || (a[0]==2 && ((hardWare.tmpgrf[a[2]]+a[3])&1)==0) ||
			(a[0]==3 && ((hardWare.tmpgrf[a[2]]+a[3])&1)==0) || (a[0]==4 && ((hardWare.tmpgrf[a[2]]+a[3])&3)==0)))
			doit(a[0],a[1],a[2],a[3]),hardWare.tmppc+=4;
		else {
			a[2] = 0;
			a[3] = ((rand()%40)<<2);
			doit(a[0],a[1],a[2],a[3]);
		...
		if(hardWare.tmpgrf[a[2]]+a[3]<0||hardWare.tmpgrf[a[2]]+a[3]>1023){
			a[3] = -(hardWare.tmpgrf[a[2]]);
			doit(a[0],a[1],a[2],a[3]);
			hardWare.tmppc+=4;
			return true;
		}
...
struct mips_sTypeSet//id=4
{
	...
	inline void debug()
	{
		...
		{
			hardWare.tmpdm[((a+b)&255)>>2]=0x11111111;
			if (n==-1) return;
	...
	inline bool trydo(int a[])
	{
		/*if(hardWare.tmpgrf[a[2]]>=67107840 || hardWare.tmpgrf[a[2]]<=-67107840){
			a[2] = 0;
			a[3] = abs(rand()%50+70)<<2;
			doit(a[0],a[1],a[2],a[3]);
			hardWare.tmppc+=4;
			return true;
		}*/
		a[3]=((rand()%50+70)<<2)-(hardWare.tmpgrf[a[2]]);
		/*
		if ((a[3]&0xffff8000)!=0 && ((~a[3])&0xffff8000)!=0)
		{
			if (DEBUGMOOD) fprintf(out,"failed %s $%d,%d($%d)\n",
		...
		if ((0<=(hardWare.tmpgrf[a[2]]+a[3]) && (hardWare.tmpgrf[a[2]]+a[3])<=0x2fff) &&
			((a[3]&0xffff8000)==0 || ((~a[3])&0xffff8000)==0) &&
			(a[0]==0 || (a[0]==1 && ((hardWare.tmpgrf[a[2]]+a[3])&1)==0) || (a[0]==2 && ((hardWare.tmpgrf[a[2]]+a[3])&3)==0)))
			doit(a[0],a[1],a[2],a[3]),hardWare.tmppc+=4;
		else {
			a[2] = 0;
			a[3] = ((rand()%40)<<2);
...
inline void newfile(int num,bool debug)
{
	...
	{
		sprintf(buffer,"testpoint%d.asm",fprtot++);
...
inline bool check(int index)
{
	for (int i=0;i<4;i++)
	{
		if (block[index].instrType[i]==0) rTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==1) i1TypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==2) i2TypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==3) {lTypeSet.trydo(block[index].instr[i]);}
		else if (block[index].instrType[i]==4) {sTypeSet.trydo(block[index].instr[i]);}
		else if (block[index].instrType[i]==5) mtTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==6) mfTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==7) bdyTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==8) bdnTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==9) bsyTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==10) bsnTypeSet.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==11) luiSpecial.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==12) jrSpecial.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==13) jalSpecial.trydo(block[index].instr[i]);
		else if (block[index].instrType[i]==14) jalrSpecial.trydo(block[index].instr[i]);
...
{
	for (int i=0;i<4;i++)
	hardWare.tmppc+=(block[index].instrType[i]==14)<<2;
	return;
}
...
inline void printBlock(int index)
{
	...
	if (check(index)) print(index),hardWare.update();
	else
	{
