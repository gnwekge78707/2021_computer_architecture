...
for j in range(1):
	...
	os.system("java -jar Mars.jar db nc mc CompactDataAtZero dump .text HexText data1.txt >out_std.txt 10000 "+"test.asm")
	print("std done")
		...
		with open("out_std.txt","r") as std:
			while(1):
				...
				if (asm[0]!="j" or asm[0]=="jal") and asm[0]!="b":
					...
					if l1[89]=="1":
						l1_rtl = "@%s: *%s <= %s"%(b_to_h_pc(l1[138:177]),b_to_h(l1[91:97]),b_to_h(l1[98:137]))
					if l1[40]=="1":
