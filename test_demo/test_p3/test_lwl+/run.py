...
def b_to_h(a):
	...
	for char in a:
		if not(char==" "):
			bcode += char
	hcode = hex(int(bcode,2))
	hcode1 = hcode[2:]
	hcode2 = hcode1.zfill(8)
	return hcode2
def b_to_d(a):
	bcode = ""
	for char in a:
...
def b_to_h_pc(a):
	...
	for char in a:
		if not(char==" "):
			bcode += char
	hcode = hex(int(bcode,2)+0x00003000)
	hcode1 = hcode[2:]
	hcode2 = hcode1.zfill(8)
	return hcode2
def b_to_h_dm(a):
...
for j in range(1):
	...
	os.system("java -jar logisim.jar source.circ -tty table >out_source.txt 10000")
	print("source done")
	
	#os.system("java -jar logisim.jar std.circ -tty table >out_std.txt 100")
	os.system("java -jar Mars.jar  nc mc CompactDataAtZero dump .text HexText data1.txt >out_std.txt 10000 "+"test.asm")
	print("std done")
	...
	with open("out_source.txt","r") as source:
		with open("out_std.txt","r") as std:
			while(1):
				...
				#l2_rtl = ""
				if asm!="":
					if (asm[0]!="j" or asm[0:3]=="jal") and asm[0]!="b":


						if l1[89]=="1":
							l1_rtl = "@%s: *%s <= %s"%(b_to_h(l1[138:177]),b_to_h_dm(l1[91:97]),b_to_h(l1[98:137]))
						if l1[40]=="1":
							if(b_to_d(l1[42:48])==" 0"):
								continue
							l1_rtl = "@%s: $%s <= %s"%(b_to_h(l1[138:177]),b_to_d(l1[42:48]),b_to_h(l1[49:88]))

						l2=std.readline().strip()
						if(l1_rtl!=l2):
							print("WA at line:%d"%(i)+"	source::"+l1_rtl+"		std::"+l2+"		asm::"+asm)
						else:
							print("AC at line:%d "%(i)+"	source::"+l1_rtl+"		asm::"+asm)
				'''
				else :
					if l1[89]=="1":
						l1_rtl = "@%s: *%s <= %s"%(b_to_h_pc(l1[138:177]),b_to_h_dm(l1[91:97]),b_to_h(l1[98:137]))
					if l1[40]=="1":
						if(b_to_d(l1[42:48])==" 0"):
							continue
						l1_rtl = "@%s: $%s <= %s"%(b_to_h_pc(l1[138:177]),b_to_d(l1[42:48]),b_to_h(l1[49:88]))
