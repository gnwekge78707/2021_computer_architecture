...
def run_test():
    ...
    if not _img_out.closed:
        _img_out.close()
    logisim_path = pathlib.PurePath(os.path.dirname(os.path.realpath(__file__))) /  'logisim.jar'
    output = subprocess.check_output(['java', '-jar', logisim_path, '-tty', 'table', '-load', os.path.realpath(_img_out.name), 'cpu2.circ'], text=True)
