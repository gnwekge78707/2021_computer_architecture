...
for lll in range(132,290):
    testflie = lll;
    #os.system("test_gen")
    #Sasmfilename=("test3.asm")
...
#------------------------------------------------------------------------------------------ mips2
    ...
    #print("std done")
    os.system(xlinx+"\\bin\\nt64\\fuse "+"--nodebug  --prj mips.prj -o mips.exe mips_tb >log.txt")
    os.system("mips.exe -nolog -tclbatch mips.tcl >out_std.txt")
    #print("source done")
    path_mips2 = path
...
#------------------------------------------------------------------------------------------ cmp
    process=0
    with open(path_mips1+"out_source.txt","r") as my:
        lines=my.readlines()
        if(len(lines)==0):
    ...
    while(1):
        if(lines[n][9]=="@"): 
            ...
            n=n+1
    if(process):
        with open(path_mips1+"out_source.txt","w") as my:
            my.writelines(lines[n:])

    process=0
    with open(path_mips2+"out_std.txt","r") as my:
        lines=my.readlines()
        if(len(lines)==0):
            print("fail to simulate")
            os._exit(1)
    ...
    while(1):
        if(lines[n][9]=="@"): 
            ...
            n=n+1
    if(process):
        with open(path_mips2+"out_std.txt","w") as my:
            my.writelines(lines[n:])

    i=0
    biao=0
    with open(path_mips1+"out_source.txt","r") as source:
        with open(path_mips2+"out_std.txt","r") as std:
            while(1):
                i+=1
